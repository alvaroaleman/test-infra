...
func splitOrgRepoString(orgRepo string) (string, string, bool) {
	...
	if len(split) != 2 {
		...
		logrus.WithField("OrgRepoStrig", orgRepo).Warn("OrgRepo string is invalid, will be ignored")
		return "", "", false
	}
...
func (tq *TideQuery) OrgQueries() map[string]string {
	...
	for org, repoIdentifiers := range orgRepoIdentifiers {
		if result[org] == "" {
			result[org] = fmt.Sprintf(`%s org:"%s"`, queryString, org)
		}
		result[org] += " " + strings.Join(repoIdentifiers, " ")
	}
...
func (tq *TideQuery) Query() string {
	...
	for org, repoIdentifiers := range orgRepoIdentifiers {
		toks = append(toks, fmt.Sprintf("org:\"%s\"", org))
